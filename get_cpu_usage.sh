...
do
    ...
    if [[ "$cpu" == "cpu" ]]; then
        OUTPUT="Total: $CPU_Percentage"
        OUTPUT2="total=$CPU_Percentage"
    else
        if [ ! $(echo "$CPU_Percentage > $WARNLIMIT" | bc -l) -eq 0 ]; then
                WARN=1
        fi
        if [ ! $(echo "$CPU_Percentage > $CRITLIMIT" | bc -l) -eq 0 ]; then
                CRIT=1
        fi
        OUTPUT="$OUTPUT, $cpu $CPU_Percentage%"
        OUTPUT2="$OUTPUT2 $cpu=$CPU_Percentage"
    fi
done
